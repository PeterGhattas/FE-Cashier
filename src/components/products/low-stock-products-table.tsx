...
import { useMyLowStockProductsQuery } from "@/graphql/operations/products/myLowStockProducts.generated"
...
export function LowStockProductsTable() {
  ...
  const { data, loading, error } = useMyLowStockProductsQuery()

  const products = React.useMemo(() => {
    if (!data?.myLowStockProducts) return []

    if (!searchQuery) return data.myLowStockProducts

    const query = searchQuery.toLowerCase()
    return data.myLowStockProducts.filter((product) =>
      product.name.toLowerCase().includes(query) ||
      product.sku.toLowerCase().includes(query) ||
      product.barcode?.toLowerCase().includes(query) ||
      product.category?.toLowerCase().includes(query)
    )
  }, [data?.myLowStockProducts, searchQuery])
          ...
          <CardContent>
            {products.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12 text-center">
                ...
                <h3 className="text-lg font-semibold mb-2">No low stock products</h3>
                        ...
                        <TableCell className="text-right">
                          <div className="flex flex-col items-end gap-1">
                            ...
                            <div className="w-16 h-1.5 bg-muted rounded-full overflow-hidden">
                              <div
                                className={`h-full ${
                                  stockPercentage < 50 ? "bg-red-500" : "bg-orange-500"
                                }`}
